...
bool IGraphicsMac::DrawScreen(IRECT* pR)
{
    ...
    bool upscaleImage = true;
    if (mPlug->GetGUIResize() && IsUsingSystemGUIScaling())
    {
        upscaleImage = false;

        CGContextScaleCTM(pCGC, 1.0 / GetSystemGUIScaleRatio(), 1.0 / GetSystemGUIScaleRatio());
    }
    
    if (false && upscaleImage && CGContextConvertSizeToDeviceSpace(pCGC, CGSizeMake(1,1)).width > 1.9)
    {
